...
``` {.c #lst-sumatoria-h lst-cap="sumatoria.h"}
...
``` {.c #lst-sumatoria-main lst-cap="main.c"}
...
:::{.callout appearance="minimal"}
``` {.c #lst-sumatoria-c lst-cap="sumatoria.c"}
#include <sumatoria.h>
...
:::{.callout appearance="minimal"}
```{.asm #lst-sumatoria-asm lst-cap="sumatoria.s"}
  .syntax unified
  .thumb
  .cpu cortex-m3
  .fpu softvfp
...
:::{.callout appearance="minimal"}
``` {.hex #lst-sumatoria-dump lst-cap="Volcado de memoria del símbolo *sumatoria*" code-line-numbers="false"}
...
``` {.hex #lst-sumatoria-disassemble lst-cap="Desensamblado de *sumatoria*" code-line-numbers="false"}
08000340 <sumatoria>:
...
:::{.callout appearance="minimal"}
```{.c #lst-parpadeo-c lst-cap="parpadeo.c"}
...
:::{.callout appearance="minimal"}
```{.c #lst-ejemplo-macros lst-cap="Ejemplo de macros (NO ES UN USO RECOMENDADO)"}
#include <stm32f1xx.h>
...
```
```{.c #lst-ejemplo-macros-expansion lst-cap="Macros expandidas (lineas quebradas para que el código entre en el espacio)" code-line-numbers="false"}
int main(void){
...
:::{.callout appearance="minimal"}
``` {.c #lst-comp-condicional lst-cap="Ejemplo compilación condicional"}
...
:::{.callout appearance="minimal"}
```{.c #lst-ejemplo-encabezado lst-cap="sp_tiempo.h"}
#ifndef SP_TIEMPO_H
#define SP_TIEMPO_H
