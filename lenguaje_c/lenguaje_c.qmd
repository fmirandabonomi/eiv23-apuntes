...
reserva memoria para el más grande entre a,b y c. Puede contener a **o** b **o** c, pero **solo uno por vez**. Al usar uniones lo mejor es ubicarlas dentro de estructuras donde otro miembro determine *cual de los valores es contenido*. El @lst-variant-union muestra un ejemplo de union asociada a un identificador de tipo.

:::{.callout appearance="minimal"}
::::{#lst-variant-union lst-cap="Union e identificador de tipo"}
``` c
typedef struct Var{
...
El tipo función es una clase de tipo derivado, donde el tipo base identifica el tipo retornado por la función, y además se especifican otros tipos que corresponden a los argumentos de la función. Un declarador de función se construye ubicando una lista de argumentos entre paréntesis a la derecha del identificador. Si una función no retorna valor su tipo base debe ser *void*. Y si no acepta argumentos la lista de argumentos debe contener solo uno, de tipo *void*. El @lst-declaracion-de-funciones muestra ejemplos de declaración de funciones. El @lst-declara-fn-retorna-struct muestra un ejemplo de declaración de función que retorna un valor tipo *struct*. Una función no puede retornar un valor arreglo, pero sí *struct* (y un *struct* puede contener un arreglo). No es recomendable retornar valores *struct* grandes puesto que deben pasarse en la pila de llamados y puede dar lugar a un desborde de pila, sobre todo en sistemas embebidos donde no hay grandes cantidades de RAM.
...
:::{.callout appearance="minimal"}
::::{#lst-declaracion-de-funciones lst-cap="Ejemplos de delaración de funciones"}
``` c
/*
 * Declaración (llamada prototipo) de función de dos argumentos enteros que
 * retorna un entero (el tipo base es el tipo del valor retornado).
 * Notar la claridad adicional que brinda el incluir los nombres de los
 * argumentos y elegir bien el nombre de la funcion
 */
int depositaEnCuenta(int idCuenta, int cantidad);
/* 
 * Declaración de múltiples funciones con el mismo tipo base y sin dar nombre a
 * los argumentos (VÁLIDO PERO NO RECOMENDADO)
 */
...
::: callout-note
Si bien el @lst-declaracion-de-funciones declara dos funciones en con el mismo especificador de tipo base, es conveniente por claridad declarar cada función por separado.
:::

:::{.callout appearance="minimal"}
::::{#lst-declara-fn-retorna-struct lst-cap="Declaración de función que retorna valor *struct*"}
``` c
...
::: callout-note
En el @lst-declara-fn-retorna-struct, el tipo `Opc_ResultadoDiv` comienza su nombre por Opc\_ por *opcional*. En caso de que `es_resultado` sea *falso* significa que ocurrió un problema (divisor cero) y no fue posible determinar el resultado. Este clase de tipo de estructura Op\_... es práctico en casos donde puedan ocurrir errores.
:::

Si el tipo de una declaración de función tiene el especificador *static*, se está declarando una función visible solo dentro de la unidad de traducción (archivo .c y encabezados) actual (llamado *vinculación local*). En ese caso la función debe ser definida en la misma unidad de traducción donde fue declarada. El @lst-declara-fn-static muestra un ejemplo de declaración de función con *vinculación local*. Tener en cuenta que la función debe ser definida en la misma unidad de traducción donde fue declarada.

:::{.callout appearance="minimal"}
::::{#lst-declara-fn-static lst-cap="Declaración de función con *vinculación local* (*static*)"}
``` c
...
::: callout-caution
En el @lst-declara-fn-static sería un error si termina el archivo sin definir func1.
:::

## Funciones inline

En C99 se introdujo un especificador `inline` que indica que el código de la función puede ser "pegado" dentro de otra función en lugar de hacer un llamado. Esto permite evitar la sobrecarga de llamar a una función sencilla. El @lst-declaracion-implementacion-fn-inline muestra un ejemplo de función inline.

::: callout-tip
El compilador de C solo hace la inclusión en línea del código **si está activada la optimización**. Si no está activada la optimización (en GCC si se usara la opción -O0 o -fno-inline) el compilador realizará un llamado normal en su lugar. En GCC las opciones -finline-functions y -finline-small-functions por ejemplo instruyen al compilador que considere todas las funciones o las funciones pequeñas (comparables al espacio ocupado para llamarlas) como si fuesen declaradas *inline* y las inserte cuando tenga la definición antes del llamado, siempre que no exceda ciertas métricas internas del compilador (para evitar un crecimiento excesivo del espacio ocupado).
:::
...
:::{.callout appearance="minimal"}
::::{#lst-declaracion-implementacion-fn-inline lst-cap="Declaración e implementación de función *inline*"}
*ejemplo.h*
``` c
#ifndef EJEMPLO_H
#define EJEMPLO_H
...
Puede forzarse al compilador a incluir en línea siempre una determinada función. Esto no es una característica estándar pero está presente en la mayoría de los compiladores. En el caso de GCC debe utilizarse el atributo `__always_inline__`. El @lst-declaracion-fn-always-inline muestra un ejemplo de función que *siempre que sea posible* es incluida en línea en GCC.

:::{.callout appearance="minimal"}
::::{#lst-declaracion-fn-always-inline lst-cap="Función que se incluye en línea siempre que sea posible (para GCC)"}
*maximo.h*
```c
#ifndef MAXIMO_H
#define MAXIMO_H
...
Un arreglo es un tipo de datos compuesto que contiene varias instancias del tipo de dato base dispuestas en forma contigua en memoria. El declarador de arreglo es indicado por un par de corchetes a la derecha del resto del declarador. Dentro de los corchetes se indica el número de elementos del arreglo. Un arreglo reserva memoria para todos sus elementos. El @lst-declaracion-arreglos muestra un ejemplo de declaración de arreglo.

:::{.callout appearance="minimal"}
::::{#lst-declaracion-arreglos lst-cap="Declaración de arreglos"}
``` c
  int var1[5], var2[3][2];
  /*
...
:::{.callout-note}
En el operador indexado se pueden invertir los roles de puntero e índice (`a[b]` y `b[a]` tienen el mismo efecto), **sin embargo esto debe evitarse porque hace el código ilegible**. Esta nota tiene por finalidad poner sobre aviso al lector en caso que encuentre dicho uso en algún código fuente (usualmente constituirá intento de ofuscación de parte del autor de dicho código). Para entender el porqué de esto basta con considerar que `x[y]` equivale a `(*(x+y))`.
:::

## Arreglos como argumentos de funciones

Cuando un arreglo participa en una expresión es evaluado a un puntero a su primer elemento, esto ocurre también como parte de un llamado a función. Si uno de los argumentos en el llamado a función es especificado como un arreglo equivale a especificar un puntero al primer elemento. El @lst-ejemplo-arreglo-como-argumento muestra ejemplos de funciones que operan sobre arreglos. Notar que sus argumentos son en realidad punteros, incluso si se utilizó notación de arreglo al declararlos.

:::{.callout appearance="minimal"}
::::{#lst-ejemplo-arreglo-como-argumento lst-cap="Funciones que operan sobre arreglos"}
``` c
...
El @lst-punteros-y-arreglos ilustra estas diferencias.
:::

:::{.callout appearance="minimal"}
::::{#lst-punteros-y-arreglos lst-cap="Punteros y arreglos"}
``` c
...
Un puntero es un tipo derivado cuyo valor es una dirección de memoria que, a su vez, contiene un objeto del tipo base. El tipo base puede ser cualquier otro tipo. El @lst-declaraciones-de-punteros muestra declaraciones de punteros.
...
:::{.callout appearance="minimal"}
::::{#lst-declaraciones-de-punteros lst-cap="Declaraciones de punteros"}
``` c
int x=5, *a=&x, **b=&a, ***c=&b;
/* x es una variable entero
 * a es una variable puntero a entero, inicializada con la dirección de x
 * b es una variable puntero a puntero a entero, inicializada con la dirección
 *   de a
 * c es una variable puntero a puntero a puntero a entero, inicializada con la
 *   dirección de b
 */
 int *x[4], (*y)[6];
 // x es un arreglo de cuatro punteros a entero.
 // y es un puntero a arreglo de cuatro enteros. Notar los paréntesis. El 
 // declarador de arreglo tiene mayor precedencia que el de puntero
```
::::
:::

::: callout-tip
Notar en el @lst-declaraciones-de-punteros la diferencia entre x e y. La variable x almacena cuatro punteros a entero, mientras que y es un puntero a un arreglo de seis enteros.
...
Los arreglos de tipo caracter pueden inicializarse con un literal de cadena de caracteres (una cadena entre comillas dobles). Tanto los arreglos como las estructuras pueden inicializarse con una lista de inicialización, que es una lista entre corchetes con los valores de cada celda del arreglo o miembro de la estructura en el orden que fueron declarados. En el caso que un miembro o celda sea a su vez un arreglo o estructura se especificará con su propia lista de inicialización. La inicialización de unión es de la misma forma, pero en ese caso solo puede especificarse el primer miembro (en la unión solo puede haber un miembro válido por vez, y con este tipo de inicializador debe ser el primero).

Desde C99 pueden utilizarse designadores en las listas de inicialización. El designador está a la izuierda de un valor e indica a que celda del arreglo o miembro de la estructura o unión inicializa. El designador de celda de arreglo consiste en el índice de la celda entre corchetes seguido de un signo igual. El designador de miembro de unión o estructura es un punto seguido del nombre del miembro y el signo igual.

El @lst-inicializadores-arreglo-estructura muestra ejemplos concretos de inicializadores de estructuras, arreglos y uniones. 

:::{.callout appearance="minimal"}
::::{#lst-inicializadores-arreglo-estructura lst-cap="Inicializadores de arreglo y estructura"}
``` c
typedef struct MiStruct {int a;int b;int c;} MiStruct;
typedef union MiUnion {char b[4];short h[2];long w;} MiUnion;
// Lista de inicialización
// Toma la dimensión de la lista
int arreglo_1[] = {1,2,3,4}; 
// Los miembros no especificados se inicializan en 0
int arreglo_2[8] = {1,2,3,4};
// Solo puede omitirse la primera dimensión
int arreglo_3[][3] = {{1,2},{1,3,4}};
// equivale a {'A','B','C','0'}
char arreglo4[] = "ABC";
// a=3, b=5, c=7
MiStruct estructura_1 = {3,5,7};
// a=1, b=0, c=0
MiStruct estructura_2 = {1};
// En una unión solo puede inicializarse así el primer miembro
MiUnion  union_1 = {{1,2,3,4}};
...
Los literales compuestos crean objetos de tipo estructura, unión o arreglo. Para crearlos se especifica el tipo entre paréntesis y se ubica a continuación una lista de inicialización (idéntica a la utilizada para inicializar una variable). El @lst-literales-compuestos tiene ejemplos de literales compuestos.
...
:::{.callout appearance="minimal"}
::::{#lst-literales-compuestos lst-cap="Literales compuestos"}
``` c
#include <assert.h>
...
Son los operadores de post-incremento (`x++`), post-decremento (`x--`), llamado a función (`fn(2,4)`), direccionamiento de arreglo (`x[3]`), acceso a miembro (`x.y`), acceso a miembro por puntero (`x->y`) y literal compuesto (`(int[3]){1,2,3}`). Se ejecutan antes que cualquier otro operador (excepto agrupación con paréntesis).

::: callout-note
La asociatividad de estos operadores es de ***izquierda a derecha***. Ejemplos:\
`x.y.z` equivale a `(x.y).z`\
`x->y++` equivale a `(x->y)++`\
`fx(a)(b)` equivale a `(fx(a))(b)` (fx retorna un puntero a función)\
`x[3][2]` equivale a `(x[3])[2]`
:::

:::{.callout appearance="minimal"}
::::{#lst-ejemplo-operadores-precedencia-1 lst-cap="Ejemplo de operadores de precedencia 1"}
``` c
#include <stdio.h>
#include <assert.h>
...
Son operadores de pre-incremento (`++x`), pre-decremento (`--x`), signos positivo (`+x`) y negativo (`-x`), NOT booleano (`!x`), NOT bit a bit (`~x`), conversión de tipo (`(int)x`), indirección (acceso al destino de un puntero, `*x`), dirección (dirección de memoria de un objeto, `&x`) y *sizeof* (tamaño de un objeto, `sizeof x`). Son evaluados inmediatamente después de evaluar los operadores de precedencia 1.

::: callout-note
La asociatividad de estos operadores es de ***derecha a izquierda***. Ejemplos:\
`(unsigned)-x` equivale a `(unsigned)(-x)`\
`**x` equivale a `*(*x)`\
`*++*y` equivale a `*(++(*y))`\
:::

:::{.callout appearance="minimal"}
::::{#lst-ejemplo-operadores-precedencia-2 lst-cap="Ejemplo de operadores de precedencia 2"}
``` c
#include <assert.h>
#include <stdint.h>
int main(void){
...
## Operadores de precedencia 3

Son los operadores producto (`x*y`), cociente (`x/y`) y resto (`x%y`). Son evaluados inmediatamente después de los operadores de precedencia 2.

:::{.callout-note}
La asociatividad de estos operadores es de ***izquierda a derecha***. Ejemplos:\
`x*y/z` equivale a `(x*y)/z`\
`x/y*z%t` equivale a `((x/y)*z)%t`
:::

:::{.callout appearance="minimal"}
::::{#lst-ejemplo-operadores-precedencia-3 lst-cap="Ejemplo de operadores de precedencia 3"}
``` c
#include <assert.h>
#include <stdint.h>
int main(void){
...
## Operadores de precedencia 4

Son los operadores suma (`x+y`) y resta (`x-y`). Se evalúan inmediatamente después que los de precedencia 3 (producto, cociente y módulo).

:::{.callout-note}
La asociatividad de estos operadores es de ***izquierda a derecha***. Ejemplo:\
`x-y+z` equivale a `(x-y)+z`\
:::

## Operadores de precedencia 5

Son los operadores desplazamiento de bit a izquierda (`x<<y`) y derecha (`x>>y`). Se evalúan inmediatamente *después que los operadores suma y resta*.
...
:::{.callout-warning}
La precedencia de estos operadores puede ser anti-intuitiva. Ejemplo:\
`x<<y+3` equivale a `x<<(y+3)`\
`x+3<<y` equivale a `(x+3)<<y`

Recomendamos utilizar paréntesis explícitamente siempre que se utilizen este tipo de operador. Ejemplos:\
`x<<(y+3)` es más claro\
`x+(3<<y)` es probablemente lo que se quería significar antes
:::

## Operadores de precedencia 6

Son los operadores relacionales, menor que (`x<y`), menor o igual que (`x<=y`), mayor que (`x>y`) y mayor o igual que (`x>=y`). El resultado de la evaluación de estos operadores es un valor booleano (`0` para falso o `1` para verdadero).

:::{.callout-warning}
Usualmente no debería haber operadores relacionales consecutivos. Dicho esto, la asociatividad de estos operadores es de ***izquierda a derecha***. Si no se tiene cuidado puede dar lugar a comportamientos anti-intuitivos o sorprendentes. Ejemplo:\
`3<2<1` equivale a `(3<2)<1`, que evalúa a `1<1` y luego `0`.

Además, los operadores menor o igual que y mayor o igual que son formados por dos caracteres. Si se insertara un espacio daría lugar a un error.
:::

## Operadores de precedencia 7

Son los operadores igual que (`x==y`) y distinto que (`x!=y`). Se evalúan luego de evaluar los operadores relacionales.

:::{.callout-warning}
Es facil confundir el operador igual que `==` con el operador de asignación `=` lo cual puede dar lugar a errores lógicos en la programación. La asociatividad es de ***izquierda a derecha***, pero nuevamente **no debería haber operadores de este tipo seguidos**. Ejemplo:\
`1==3==0` equivale a `(1==3)==0` que evalúa a `0==0` y `1`  
:::

## Operador de precedencia 8

Es el operador de conjunción (Y, AND) bit a bit. Tiene la forma `x&y`. El i-ésimo bit del resultado es la conjunción de los i-ésimos bit de los operandos $$(x\&y)_{<i>}=x_{<i>} \wedge y_{<i>}$$

:::{.callout-note}
La asociatividad de este operador es de *izquierda a derecha*, pero como es el único en su precedencia y se trata de una operación asociativa, no es relevante en la práctica.
:::

:::{.callout-warning}
Este opreador se evalúa luego que los relacionales y de igualdad, esto puede dar lugar a comportamientos inesperados. Cuando aparecen estos operadores en la misma expresión debe utilizarse paréntesis explícitos por claridad. Ejemplo:`
`x&7<3` equivale a `x&(7<3)` que evalúa a `x&0` o `0`. Seguramente esto no es lo buscado.
`(x&7)<3` compara el valor de los primeros 3 bit de x con 3, que es seguramente lo buscado en ese caso.
:::

## Operador de precedencia 9

Es el operador de disyunción exclusiva (O exclusivo, XOR) bit a bit. Tiene la forma `x^y`. El i-ésimo bit del resultado es la disyunción exclusiva de los i-ésimos bit de los operandos $$ (x^y)_{<i>}=x_{<i>} \oplus y_{<i>} $$

:::{.callout-warning}
En otros lenguajes el operador `^` se utiliza para exponenciación. C *no tiene operador de exponenciación*. Vale la misma consideración que el operador `&` respecto del uso de paréntesis cuando en la expresión hay operaciones relacionales. 
:::

## Operador de precedencia 10

Es el operador de disyunción (O, OR) bit a bit. Tiene la forma `x|y`. El i-ésimo bit del resultado es la disyunción de los i-ésimos bit de los operandos $$ (x|y)_{<i>}=x_{<i>} \vee y_{<i>}$$

:::{.callout-warning}
Valen las mismas advertencias que para los operadores `&` y `^`.
:::

## Operador de precedencia 11

Es el operador Y lógico booleano `x&&y`. Este operador evalúa sus argumentos como valores lógicos, `0` es falso y cualquier otro valor es verdadero. Es un operador llamado de "cortocircuito", primero evalúa su argumento izquierdo y, si este es verdadero, recién evalúa su argumento derecho. Es uno de los pocos casos donde el orden de evaluación es garantizado (garantiza que si el operando izquierdo es falso *nunca evaluará el derecho*). Esta propiedad es importante para evitar realizar operaciones inválidas o acceso a punteros nulos. Siempre el pre-requisito se evalúa antes de realizar la operación y la operación no se realiza si este no se cumple. Su resultado será siempre `1` verdadero o `0` falso.

:::{.callout-warning}
Este operador se evalúa luego de todos los operadores relacionales, aritméticos y lógicos bit a bit. Esto es lo que se espera normalmente, sin embargo es siempre conveniente usar paréntesis cuando debe aclararse el orden de operaciones. Además siempre usar paréntesis si se mezclan el operador O booleano (`x||y`) con este operador. Ejemplos:\
`x>3 && y<2` equivale a `(x>3) && (y<2)`, que es lo que se espera normalmente. Y si `x<3` fuese falso no se evaluaría `y<2`.\
`x<3 && y<2 || z<4 && k>1` equivale a `((x<3) && (y<2)) || ((z<4) && (k>1))` pero esto no es claro.\
`(x<3 && y<2) || (z<4 && k>1)` es una expresión más clara y preferible a la anterior
:::

:::{.callout-note}
La asociatividad del operador es de ***izquierda a derecha***, y garantiza el orden de evaluación. Ejemplos:\
`x && fn && fn(z)/x > 2` equivale a `(x && fn) && (fn(z)/x > 2)`, y la última expresión solo se ejecutará si `x` y `fn` (puntero a función) son distintos de cero.
:::

## Operador de precedencia 12

Es el operador O lógico booleano `x||y`. Se evalúa luego del operador Y lógico booleano y tiene una propiedad de evaluación análoga. Si el argumento izquierdo es verdadero *no se evalúa el argumento derecho*. Su resultado será siempre `1` verdadero o `0` falso.

:::{.callout-warning}
Vale la misma advertencia que para el operador `&&`
:::

:::{.callout-note}
La asociatividad del operador es de ***izquierda a derecha***, y garantiza el orden de evaluación. Ejemplos:\
`x == 0 || 3/x > 5` equivale a `(x == 0) || (3/x > 5)`, y la última expresión no se ejecuta en caso que x sea cero.
:::

## Operador de precedencia 13

Es el operador condicional (`expr1 ? expr2 : expr3`). Es un operador ternario (de tres argumentos). Primero evalúa su primer argumento (`expr1`). Si el resultado es *verdadero* entonces evalúa el segundo argumento y retorna su valor (*y no evalúa el tercero*). Si el resultado es *falso* entonces evalúa el tercer argumento y retorna su valor (*y no evalúa el segundo*). Este opreador se evalúa luego de los operadores lógicos booleanos, así que en la mayoría de los casos es innecesario el uso de paréntesis. Suele ser conveniente, sin embargo, utilizar paréntesis en el primer argumento por claridad. Si el operador participa como parte de una expresión más grande deberemos siempre rodearlo de paréntesis.

:::{.callout-note}
La asociatividad del operador condicional es de ***derecha a izquierda***. Ejemplo:\
`x ? 1 : y ? 4 : 2` equivale a `x ? 1 : (y ? 4 : 2)`\
`x ? y ? 5 : 7 : 3` equivale a `x ? (y ? 5 : 7) : 3`\
`x ? y ? 3 : 2 : z ? 4 : 1` equivale a `x ? (y ? 3 : 2) : (z ? 4 : 1)`

Al ser un operador ternario es más dificil evaluar la asociatividad. Para hacerlo buscamos de derecha a izquierda los signos de interrogación y rodeamos los operandos asociados en paréntesis.

El operador condicional es una mejor opción que el uso de estructuras condicionales (if-else) cuando el único efecto es determinar un valor. El @lst-ejemplo-operador-condicional muestra un ejemplo de este uso.
:::

:::{.callout appearance="minimal"}
::::{#lst-ejemplo-operador-condicional lst-cap="Ejemplo de uso operador condicional"}
```c
enum ClaseCaracter{
  LETRA_MAYUSCULA,
  LETRA_MINUSCULA,
...
ClaseCaracter obtClase1(char a)
{
  return a >= 'A' && a <= 'Z' ? LETRA_MAYUSCULA :
         a >= 'a' && a <= 'z' ? LETRA_MINUSCULA :
         a >= '0' && a <= '9' ? NUMERO          :
         a < 32 || a == 127   ? ESPECIAL        :
         a > 127              ? EXTENDIDO       :
                                PUNTUACION;
}
// Exactamente igual a obtClase1, explicita agrupación con paréntesis
ClaseCaracter obtClase2(char a)
{
  return a >= 'A' && a <= 'Z' ? LETRA_MAYUSCULA :
         (a >= 'a' && a <= 'z' ? LETRA_MINUSCULA :
          (a >= '0' && a <= '9' ? NUMERO :
           (a < 32 || a == 127 ? ESPECIAL :
            (a > 127 ? EXTENDIDO :PUNTUACION))));
}
```
::::
:::

## Operadores de precedencia 14

Son los operadores de asignación (`x=1`) y asignación compuesta (`x+=1`, `x/=3`, etc.). Estos operadores se evalúan luego del operador condicional y todos los demás operadores excepto el operador coma `,`. Su operando izquierdo debe ser un objeto al cual pueda asignarse un valor (en terminología de C se denomina *l-valor modificable*). Un l-valor modificable puede ser *una variable no const*, *el resultado de dereferenciar un puntero (o arreglo) no const*, *el resultado de indexar un puntero (o arreglo) no const* o *el resultado del acceso a un miembro de estructura o unión que no sea const*. El efecto del operador asignación es asignar el valor resultante de evaluar su operando derecho como nuevo valor del objeto identificado por su operando izquierdo. El valor de la expresión de asignación en sí es el mismo valor que se usó en la asignación. Por ejemplo, `2*(x=3)` asigna a la variable `x` el valor `3` y produce el valor `6`.

Los operadores de asignación compuesta son `+=`, `-=`, `*=`, `/=`, `%=`, `>>=`, `<<=`, `&=`, `|=` y `^=`. Estos operadores realizan la operación indicada por el operador a la izquierda del signo igual entre el valor actual del objeto a modificar y el valor del operando derecho, el resultado es usado como nuevo valor del objeto y valor final de la expresión.

:::{.callout-tip}
La asociatividad de los operadores de asignación y asignación compuesta es de ***derecha a izquierda***. Además debemos recordar que el valor que producen es el mismo que asignan. Ejemplos:\
`x=y=z=t=3` equivale a `x=(y=(z=(t=3)))` y asigna el valor `3`a las variables `x`, `y`, `z` y `t`.\
`x/=y-=z+=t*=2` equivale a `x/=(y-=(z+=(t*=2)))` y asignará `t*2` como nuevo valor de `t`, `z+t` *con el nuevo valor de `t`* a `z`, `y-z` *con el nuevo valor de `z`* a `y`, `x/y` *con el nuevo valor de `y`* a `x`.
:::

:::{.callout-warning}
El signo igual es utilizado también para indicar la *inicialización de variables*, sin embargo **inicialización no es asignación**. La inicialización *da el valor inicial a una variable* y la asignación *modifica el valor de una variable existente*. Notablemente ***las variables `const` deben inicializarse pero no pueden ser sujetas a asignación***
:::

:::{.callout appearance="minimal"}
::::{#lst-ejemplo-asignacion lst-cap="Operadores de asignación <https://onlinegdb.com/eSQ6q_LYc>"}
```c
#include <stdio.h>
#include <assert.h>
int main(void)
{
  int x,y,z,t;
  x=y=z=t=3;
  assert(3 == x && 3 == y && 3 == z && 3 == t);
  x/=y-=z+=t*=2;
  assert(3*2 == t && 3+t == z && 3-z == y && 3/y == x);
...
## Operador de precedencia 15

El operador coma (`exp1,exp2`) es el de menor precedencia (el último en evaluarse). Evalúa en primer lugar la expresión izquierda y descarta el valor resultante. Luego evalúa la expresión derecha y produce como valor final el resultado de dicha expresión. Tiene la garantía de que siempre se evalúe primero la expresión izquierda, luego la derecha. Al usar el operador coma, la expresión izquierda es evaluada por sus efectos secundarios (operación de asignación o llamado a función).

:::{.callout-tip}
La asociatividad del operador coma es de ***izquierda a derecha***. Por ejemplo:\
`i++,j+=9,x=a[i,j]` equivale a `(i++,j+=9),x=a[i][j]`. Incrementa i, luego incrementa j y finalmente asigna a la variable x el valor resultante de las operaciones de indexado, que se efectúan con los nuevos valores de `i` y `j`
:::
